...
async def _(event):
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "ULTRON_RM_BG.png"
        ...
        await edit_or_reply(event, 
            "Removed dat annoying Backgroup in {} seconds, powered by © @its_ultron ™".format(
                ms
            )
        )
    else:
        await edit_or_reply(event, 
            "ReMove.BG API returned Errors. Please report to @its_ultron\n`{}".format(
